...
// Centralized Stripe Initialization (Safeguarded fallback if variable variations exist)
...
async function run() {
    try {
        ...
        console.log("Connected to MongoDB TicketBari Database Collections!");
    ...
    if (existingUser) {
        // Generate a fresh token with their current DB role (e.g., 'vendor')
    ...
    try {
        ...
        // 1. Look up the person exactly in the "users" collection (usersCollection)
        const dbUser = await usersCollection.findOne({ email: email });

        // 2. Fallback safely to "user" if they aren't found in the DB yet, 
        // but grab the fresh "vendor" or "admin" role if they are!
        const finalRole = dbUser ? dbUser.role : "user"; 

        // 3. Bake the real-time role straight into the token payload
        const token = jwt.sign(
   ...
   app.patch('/tickets/:id', verifyToken, async (req, res) => {
    try {
        ...
        // Destructure to remove fields that should not be updated in the database
        const { _id, createdAt, verificationStatus, vendorEmail, ...updateFields } = req.body;

        const result = await ticketsCollection.updateOne(
            { _id: new ObjectId(id) }, 
            { $set: updateFields } // Only updates the safe, modified values
        );
        ...
        app.post('/create-checkout-session', verifyToken, async (req, res) => {
            ...
            try {
                ...
                const session = await stripeLib.checkout.sessions.create({
                    ...
                    mode: 'payment',
                    success_url: `http://localhost:3000/dashboard/user?payment_success=true&bookingId=${bookingId}&ticketId=${ticketId}&qty=${quantity}&session_id={CHECKOUT_SESSION_ID}`,
        ...
        // ==========================================
        // SECURE PAYMENT VERIFICATION CONFIRMATION (REPAIRED)
        // ==========================================
        app.patch('/verify-payment', verifyToken, async (req, res) => {
            ...
            try {
                ...
                // Explicitly updates parameters to meet table requirements
